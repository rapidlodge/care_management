...
def _build_desired_tasks(doc, config):
    ...
    if explode == "weekday_row":
        ...
        for row in doc.get(config["child_table"]) or []:
            ...
            # Rich medication/shift task description (CHANGE 4)
            description_parts = [
                f"Source: {doc.doctype} {doc.name}"
            ]

            if detail_field and row.get(detail_field):
                description_parts.append(f"Dosage: {row.get(detail_field)}")

            if row.get("route"):
                description_parts.append(f"Route: {row.get('route')}")

            if row.get("instructions"):
                description_parts.append(f"Instructions: {row.get('instructions')}")
...
def sync_weekly_meal_plan_tasks(participant=None, execution_date=None):
    ...
    day_of_week = frappe.utils.get_datetime(execution_date).strftime("%A") # e.g., "Monday"

    filters = {"docstatus": 1, "is_active": 1}
    if participant:
        filters["participant"] = participant

    active_plans = frappe.get_all("Weekly Meal Planner", filters=filters, fields=["name", "participant"])

    created_count = 0
    for plan in active_plans:
        meal_items = frappe.get_all(
            "Weekly Meal Plan Item",
            filters={"parent": plan.name, "day_of_week": day_of_week},
            fields=["meal_type", "meal_name", "scheduled_time", "dietary_requirements", "preparation_instructions"]
        )

        for item in meal_items:
            # Check if instance already exists to prevent duplicate generation
            task_title = f"{item.meal_type}: {item.meal_name}"
            existing = frappe.db.exists("Support Task Execution Instance", {
            ...
            if not existing:
                inst = frappe.get_doc({
                    ...
                    "task_title": task_title,
                    "execution_date": execution_date,
                    "scheduled_start_time": item.scheduled_time or "12:00:00",
                    "status": "Scheduled",
                    "task_category": "Meal / Nutrition",
                    "instructions": f"Dietary: {item.dietary_requirements or 'None'}\nPrep: {item.preparation_instructions or 'Standard'}",
